...
\manufig{figures/figure6.png}{Annual modeled N$_2$O flux by scenario and starting land cover, 2021--2100. Line: pixel-mean flux. Ribbon: 25th--75th percentile across pixels. Units are g N$_2$O-N m$^{-2}$\,yr$^{-1}$ (confirmed from DAYCENT year\_summary.out). Barren land cover (114 pixels) excluded; biomass yield data not available.}
...
These findings characterize modeled SOC and CEC responses under a specific set of DAYCENT scenario assumptions. Several limitations constrain transferability to deployment decisions: (1) no Permian Basin field calibration of SOC or yield was performed; (2) climate forcing omits secular trends and CO$_2$ fertilization effects are disabled; (3) irrigation demand and water-resource feasibility are not evaluated; (4) CEC estimates are pedotransfer-derived proxies with high parametric uncertainty, particularly from the root-fraction depth-allocation method (0.70 applied to 0--105\,cm DAYCENT output); (5) biomass yield data are not available in the canonical results CSV; (6) layer-specific DAYCENT output needed for matched-depth SOC validation against gSSURGO is not yet available from the production runs. Future work should address calibration, climate uncertainty, water-resource constraints, layer-specific SOC extraction, and yield data recovery before drawing management recommendations.

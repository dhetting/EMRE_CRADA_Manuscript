...
% NOTE [FIG7-VERIFY resolved]: ΔCEC range updated to 1.0-7.7 cmolc/kg using validated
...
At year 2100, mean $\Delta$CEC across the 48 scenario--land-cover combinations ranges from 1.0 to 7.7\,cmol$_\text{c}$\,kg$^{-1}$ (Figs. 7 and 8). Harvest scenarios (residue removed) range from 1.0 to 4.1\,cmol$_\text{c}$\,kg$^{-1}$ (mean 2.0); no-harvest scenarios range from 2.3 to 7.7\,cmol$_\text{c}$\,kg$^{-1}$ (mean 4.4), reflecting the large 0-30 cm depth fraction ($>$\,1.0) validated for residue-retention management. The largest gain (7.7\,cmol$_\text{c}$\,kg$^{-1}$, irrigated no-harvest miscanthus on shrubland) and smallest (1.0\,cmol$_\text{c}$\,kg$^{-1}$, rainfed harvested switchgrass on cropland) span the range of soil health improvement the model projects. These changes represent 7--51\,\% of the baseline CEC median, a range that is agronomically significant: higher CEC improves buffering against pH swings, increases retention of calcium, magnesium, and potassium, and reduces leaching losses of applied fertilizers \citep{BradyWeilSoilsPlaceholder}.

The $\Delta$CEC values are estimated by a pH-dependent pedotransfer function and carry uncertainty from (1) the assumed $\textit{CEC}_\text{OM}$ coefficient, which is calibrated for temperate soils and may differ for alkaline semi-arid soils; (2) depth fractions validated from 100 shrubland pixels exclude the passive SOM pool and two land covers (cultivated, barren) owing to limited HPC re-run scope; and (3) the gSSURGO CEC field reflects a laboratory method that may not be directly comparable to PTF-derived values. Per-pixel pH distribution in the study area ranges from 4.1 to 8.6, with only 11 pixels (0.006\,\% of the domain) exceeding the PTF calibration range of pH 8.5, so PTF domain extrapolation is negligible. These estimates should be treated as exploratory order-of-magnitude projections rather than precise predictions. A full-basin re-run with per-layer output for all scenarios and land covers would replace the current 100-pixel validation sample.
...
These findings characterize modeled SOC, biomass yield, CEC, and N$_2$O responses under a specific set of DAYCENT scenario assumptions. Several limitations constrain transferability to deployment decisions: (1) no Permian Basin field calibration of SOC or yield was performed; (2) climate forcing omits secular trends and CO$_2$ fertilization effects are disabled; (3) irrigation demand and water-resource feasibility are not evaluated; (4) CEC depth fractions are validated from 100 shrubland pixels with per-layer re-runs; full-basin validation covering all land covers and scenarios requires a production-scale re-run; (5) DAYCENT does not simulate a crop establishment phase, so early-year yields may be overestimated; (6) the gSSURGO 0--30\,cm baseline cannot be compared to the 0--105\,cm DAYCENT $\Delta$SOC without a matched-depth baseline, which would require extracting 0--105\,cm gSSURGO SOC stocks. Future work should address field calibration, climate uncertainty, water-resource constraints, full-basin per-layer SOC extraction, and establishment-phase yield dynamics before drawing management recommendations.

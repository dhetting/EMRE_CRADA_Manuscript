...
The $\Delta$CEC values are estimated by a pH-dependent pedotransfer function and carry uncertainty from (1) the assumed $\textit{CEC}_\text{OM}$ coefficient, which is calibrated for temperate soils and may differ for alkaline semi-arid soils; (2) the root-fraction scaling used to derive 0--30\,cm $\Delta$SOC from 0--105\,cm $\Delta$SOC---root density fraction is not a verified proxy for the vertical distribution of SOC change, and layer-specific DAYCENT output from the production runs would be needed to compute a matched-depth $\Delta$SOC; and (3) the gSSURGO CEC field reflects a laboratory method that may not be directly comparable to the PTF-derived values. Per-pixel pH distribution in the study area ranges from 4.1 to 8.6, with only 11 pixels (0.006\,\% of the domain) exceeding the PTF calibration range of pH 8.5, so PTF domain extrapolation is negligible. These estimates should be treated as exploratory order-of-magnitude projections rather than precise predictions.
...
These findings characterize modeled SOC, biomass yield, CEC, and N$_2$O responses under a specific set of DAYCENT scenario assumptions. Several limitations constrain transferability to deployment decisions: (1) no Permian Basin field calibration of SOC or yield was performed; (2) climate forcing omits secular trends and CO$_2$ fertilization effects are disabled; (3) irrigation demand and water-resource feasibility are not evaluated; (4) CEC estimates are pedotransfer-derived proxies with high parametric uncertainty, particularly from the root-fraction depth-allocation method (0.70 applied to 0--105\,cm DAYCENT output); (5) DAYCENT does not simulate a crop establishment phase, so early-year yields may be overestimated; (6) layer-specific DAYCENT output needed for matched-depth SOC validation against gSSURGO is not available from the production runs. Future work should address field calibration, climate uncertainty, water-resource constraints, layer-specific SOC extraction, and establishment-phase yield dynamics before drawing management recommendations.

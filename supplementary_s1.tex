...
PPDF(1) & 30.0 & 30.0 & 30.0 & Minimum temperature for growth (°C) \\
PPDF(2) & 45.0 & 45.0 & 45.0 & Optimum temperature for growth (°C) \\
...
\textit{Note:} Parameters shown are the primary biological controls on biomass production and root allocation.
The full \texttt{crop.100} entry for each crop type contains 112 parameters; those not listed above are
set to DAYCENT system defaults or are not relevant to the G2-type warm-season grass parameterization.
The G2 column shows the warm-season grass base parameterization from which both \texttt{MISC} and
\texttt{SG3} were derived. Parameters that differ from the G2 base are identical for \texttt{MISC} and
\texttt{SG3} (i.e., both were updated from G2 defaults in the Wang bioenergy crop calibration).
